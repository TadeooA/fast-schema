...
var normal = {
    string: function () { return api_1.fast.string(); },
    number: function () { return api_1.fast.number(); },
    boolean: function () { return api_1.fast.boolean(); },
    array: function (schema) { return api_1.fast.array(schema); },
    object: function (shape) { return api_1.fast.object(shape); },
    // Utility methods
    literal: function (value) { return api_1.fast.literal(value); },
    any: function () { return api_1.fast.any(); },
    union: function (schemas) { return api_1.fast.union(schemas); },
    // Async support
    async: function (validator) { return api_1.fast.async(validator); },
    // Metadata
    tier: 'normal',
...
var fast_tier = {
    string: function () { return api_1.fast.wasm.hybridize(api_1.fast.string()); },
    number: function () { return api_1.fast.wasm.hybridize(api_1.fast.number()); },
    boolean: function () { return api_1.fast.wasm.hybridize(api_1.fast.boolean()); },
    array: function (schema) { return api_1.fast.wasm.hybridize(api_1.fast.array(schema)); },
    object: function (shape) { return api_1.fast.wasm.hybridize(api_1.fast.object(shape)); },
    // Batch processing
    batch: function (schema) { return api_1.fast.batch(schema); },
    // JIT optimization
    jit: function (schema) { return api_1.fast.jit(schema); },
    // Smart optimization
    smart: function (schema) { return api_1.fast.wasm.optimize(schema); },
    // Utility methods with optimization
    literal: function (value) { return api_1.fast.wasm.hybridize(api_1.fast.literal(value)); },
    any: function () { return api_1.fast.wasm.hybridize(api_1.fast.any()); },
    // Metadata
    tier: 'fast',
...
var ultra_tier = {
    string: function () { return api_1.fast.ultra.extreme.string(); },
    number: function () { return api_1.fast.ultra.extreme.number(); },
    boolean: function () { return api_1.fast.ultra.extreme.boolean(); },
    array: function (schema) { return api_1.fast.ultra.extreme.array(schema); },
    object: function (shape) { return api_1.fast.ultra.extreme.object(shape); },
    // High-performance batch processing
    batch: function (schema) { return api_1.fast.ultra.extreme.batch(schema); },
    // Pre-compilation for maximum speed
    precompile: function (schema) { return api_1.fast.ultra.optimize.precompile(schema); },
    // Bulk operations
    bulk: function (schema, data) { return api_1.fast.ultra.optimize.bulkValidate(schema, data); },
    // Memory-efficient processing
    stream: function (schema, options) { return ({
        validate: function (items) { return api_1.fast.ultra.optimize.bulkValidate(schema, items, {
            chunkSize: (options === null || options === void 0 ? void 0 : options.chunkSize) || 1000,
            parallel: true,
...
var PerformanceTierSelector = /** @class */ (function () {
    function PerformanceTierSelector() {
    }
    PerformanceTierSelector.selectTier = function (requirements) {
        var validationsPerSecond = requirements.validationsPerSecond, dataSize = requirements.dataSize, environment = requirements.environment, priority = requirements.priority;
    ...
    PerformanceTierSelector.getRecommendation = function (requirements) {
        var tier = PerformanceTierSelector.selectTier(requirements);
        return {
            tier: tier,
    ...
    PerformanceTierSelector.explainSelection = function (requirements, selectedTier) {
        var reasons = [];
        ...
        if (requirements.validationsPerSecond) {
            if (requirements.validationsPerSecond > 10000) {
                reasons.push("High throughput required (".concat(requirements.validationsPerSecond.toLocaleString(), " ops/sec)"));
            }
            else if (requirements.validationsPerSecond > 1000) {
                reasons.push("Moderate throughput required (".concat(requirements.validationsPerSecond.toLocaleString(), " ops/sec)"));
            }
        }
        ...
        return reasons.length > 0
            ? "Selected ".concat(selectedTier.tier, " tier: ").concat(reasons.join(', '))
            : "Selected ".concat(selectedTier.tier, " tier as balanced default");
    };
    PerformanceTierSelector.getAlternatives = function (selectedTier) {
        var alternatives = [];
    ...
    PerformanceTierSelector.getMigrationTips = function (tier) {
        var tips = [];
        if (tier === normal) {
    ...
    return PerformanceTierSelector;
}());
