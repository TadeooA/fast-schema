...
class NaiveValidator {
  ...
  static email(value: unknown): string {
    ...
    if (local.length === 0 || domain.length === 0) throw new Error('Invalid email');
  ...
  static object(value: unknown, shape: Record<string, (v: unknown) => any>): any {
  ...
  static array(value: unknown, itemValidator: (v: unknown) => any): any[] {
...
class JsonSchemaLikeValidator {
  ...
  private validateValue(data: unknown, schema: any, path: string): any {
...
export class Realistic100xBenchmark {
  private iterations = 100000;

  async benchmarkStringValidation(): Promise<void> {
    ...
    const fastSchema = fast.ultra.extreme.string().email().min(5).max(100);
  ...
  async benchmarkObjectValidation(): Promise<void> {
    ...
    const fastSchema = fast.ultra.extreme.object({
      id: fast.ultra.extreme.string(),
      name: fast.ultra.extreme.string(),
      email: fast.ultra.extreme.string().email(),
      age: fast.ultra.extreme.number().min(0).max(120),
      preferences: fast.ultra.extreme.object({
        theme: fast.ultra.extreme.string(),
        notifications: fast.ultra.extreme.boolean()
      })
    });
  ...
  async benchmarkArrayValidation(): Promise<void> {
    ...
    for (let iter = 0; iter < 100; iter++) {
      NaiveValidator.array(largeArray, (item) =>
        NaiveValidator.object(item, {
    ...
    const itemSchema = fast.ultra.extreme.object({
      id: fast.ultra.extreme.number(),
      name: fast.ultra.extreme.string(),
      value: fast.ultra.extreme.number()
    });
    const batchValidator = fast.ultra.extreme.batch(itemSchema);
...
export const runRealistic100xBenchmark = async (): Promise<void> => {
